...
for img_no = 1:img_skip:num_images,
    ...
    All_Metrics     = [ Scores_MIT  Scores_ABB ];
...
%***    if plot_PC_overlay && ~script_flag,
    if plot_PC_overlay,
        ...
        for pick_point_no = 1:N_pick,
            ...
            Score_string    = num2str( round(100*All_Metrics(pick_point_no,2) ));
...
%***    title('MIT Princeton Pick Points with "MIT Score : ABB Score" (RED - disagreement)')
    title(mk_str([ totename '    "MIT Score : ABB Score" (RED - disagreement)' ]))
    for pick_point_no = 1:N_pick,
        
        Pick_no_string  = num2str( pick_point_no );
        MIT_Score       = round(100*All_Metrics(pick_point_no,1) );
        ABB_Score       = round(100*All_Metrics(pick_point_no,2) );
        
        MIT_string      = num2str( MIT_Score);
        ABB_string    = num2str( ABB_Score);
