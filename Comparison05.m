...
%
%   2019-02-22      Comparison04.m Clean-up and Directory looping
%
...
for dir_no = 222:(-1):1,  % traverse directories from newest to oldest
    ...
    if num_images > 0,  % check to make sure directory is NOT EMPTY
        ...
        if exist(diary_name)==2,
            delete(diary_name);
        end
        
        summary_name      = [ 'Summary_' last_dir '.txt' ];
        if exist(summary_name)==2,
            delete(summary_name);
        end
        
        %   Set up a summary table with the following entries:
        ...
        for img_no = 1:img_skip:num_images,
            ...
            plannername      = [ img_no_text '_planner.txt' ];
            planner_file       = [ dir_content(img_no).folder '\' plannername ];
            
            yamlname      = [ img_no_text '_ppmove.result.yaml' ];
            yaml_file       = [ dir_content(img_no).folder '\' yamlname ];
            
            %   Check if ALL files exist, some folders have files missing
            %   for the last pick step etc.
            
            
            exist_tote_file      = exist(tote_file) == 2;
            exist_pixel_file     = exist(pixel_file) == 2;
            exist_planner_file   = exist(planner_file) == 2;
            exist_yaml_file      = exist(yaml_file) == 2;
            
            exist_all_files      = exist_tote_file && exist_pixel_file && exist_planner_file && exist_yaml_file;
            ...
            if exist_all_files,
                ...
                disp(['Processing image file ' plannername ]);
                planner_file_contents = importdata(planner_file);
                disp(['Processing image file ' yamlname ]);
                yaml_file_contents = importfile_yaml(yaml_file);
                ...
                %   Selected system pick point
                
                PickPoint_selected  = planner_file_contents.data(1,1:3);    % extract pick point coordinates
                
                %   Only checking x-coordinate is NOT sufficient for some
                %   files..., need to check all x, y, z
                % PickPoint_index     = find(tote_file_contents(:,1) == PickPoint_selected(1));   % find index of selected pick point
 
                distance_vectors = tote_file_contents(:,1:3) - PickPoint_selected;
                distances        = vecnorm(distance_vectors')';
                PickPoint_index     = find( distances < 1 );    % if point < 1 mm away, consider it the same
                
                %   Check if pick was successful
                Success_index       = 0;
                Error_code_index    = 0;
                for k=1:length(yaml_file_contents),
                    if yaml_file_contents{k,1}=="success",
                        Success_index       = k;
                        Error_code_index    = k+1;
                    end
                end
                
                if Success_index > 0
                    Pick_Success        = yaml_file_contents{Success_index,2}=="True";
                else
                    Pick_Success        = false;
                end
                
                Error_code          = NA_number;
                if Error_code_index > 0
                    Error_code          = yaml_file_contents{Error_code_index,2};
                end
                ...
                if Pick_Success,
                    Pick_Success_String = 'True';
                else
                    Pick_Success_String = 'False';
                end
                
                disp([ 'Success: ' Pick_Success_String ]);
                disp([ 'Error Code: ' num2str(Error_code) ]);
                ...
                %   Summarize current pick number in Summary_Table
                %             tot_num_cols    = 7;    % total number of columns in metrics tables
                %             Summary_table   = zeros(num_images,tot_num_cols);
                %
                
                %             Summary_table(imag_no, 1) = pick_point_no;
                %             Summary_table(imag_no, 2) = MIT_Score;
                %             Summary_table(imag_no, 3) = ABB_Score;
                %             Summary_table(imag_no, 4) = ;
                %             Summary_table(imag_no, 5) = pick_point_no;
                %             Summary_table(imag_no, 6) = pick_point_no;
                %             Summary_table(imag_no, 7) = pick_point_no;
                %
                
                %   ADD the pick_point_no in the LAST column of
                %   Summary_table so it is easier to use this table on
                %   its own to compare with the bin images, no need to
                %   consult the full comparison table with all the pick
                %   points
                
                %                Summary_table(img_no, :)   = All_Metrics_Percent(PickPoint_index,:);
                Summary_table(img_no, :)   = [ All_Metrics_Percent(PickPoint_index,:)  PickPoint_index ];
                
                %   Fill in the CORRECT Pick Step #, not the Pick Point
                %   number in column 1
                %   NOTE that Pick step # uses ZERO based indexing
                
                Summary_table(img_no,1)     = ( img_no - 1 );
                
                %    Modify a 0 Error_code to NA_number for formatting purposes
                if Summary_table(img_no, tot_num_cols) == 0,
                    Summary_table(img_no, tot_num_cols) = NA_number;
                end
                
            end % if exist_all_files,
        end         % for img_no = 1:num_images,
        
        diary off
        
        %   Write summary file
        %   with Summary_table contents
        
        
        diary( summary_name );
        disp([ 'Summary of directory:  ' last_dir ])
        disp(data_dir)
        disp(' ')
        col_Success         = 6;
        
        %   Note! need to check Success == 1 since it  is equal to
        %   NA_number for the failed pick steps, and the sum would then be
        %   calculated wrong
%        num_of_failures     = num_images - sum(Summary_table(:,col_Success)==1);

        num_of_failures     =  sum( Summary_table(:,col_Success)==1 | Summary_table(:,col_Success)==0 ) - sum(Summary_table(:,col_Success)==1);
    ...
    end %     if num_images > 0,  % check to make sure directory is NOT EMPTY
